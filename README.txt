...
后端建议：
	监控工具（promethues、zabbixe、estado等等），发送到slack、短信、邮件等等后，生产一份到消息队列（kafka、rabbitmq等），springboot做后端，从消息队列消费数据，自己定义规则，暴露出http://IP:PROT/monitor/info接口，返回你想要的json数据（或者我例子中这么LOW的1）